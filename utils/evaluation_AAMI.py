...
def save_accuracy_result(preds, labels, save_path):
    ...
    file_name = 'accuracy_{:.4f}.csv'.format(accuracy)
    save_file = os.path.join(save_path, file_name)
    df.to_csv(save_file, index=True)
    print("Accuracy: {:.4f} results saved in {}...".format(accuracy, save_file))
    return '{:.4f}'.format(accuracy)


def save_predictions(preds, labels, pred_probs, save_path):
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    result = np.concatenate([labels.reshape(-1, 1), preds.reshape(-1, 1), pred_probs], axis=1)
    df = pd.DataFrame(data=result, columns=['pred', 'label', '0', '1', '2', '3'])
    save_file = os.path.join(save_path, 'pred_result.csv')
