...
def create_feat_lab_patients_name(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag, add_new=False, is_intra=True, times=None, fake_train=False, fake_test=False):
    if fake_train or fake_test:
        # fake数据集做训练集
        feat_lab_patients_name = db_path + '/intra_features/' + 'intra_w_' + str(winL) + '_' + str(winR) + '_' + 'DS1'
    else:
        feat_lab_patients_name = db_path + '/intra_features/' + 'intra_w_' + str(winL) + '_' + str(winR) + '_' + DS
    # if not is_intra:
    #     feat_lab_patients_name = db_path + '/intra_features/' + 'w_' + str(winL) + '_' + str(winR) + '_' + DS
    # else:
    #     feat_lab_patients_name = db_path + '/intra_features/' + 'intra_w_' + str(winL) + '_' + str(winR) + '_' + DS

    if do_preprocess:
        feat_lab_patients_name += '_rm_bsline'

    if maxRR:
        feat_lab_patients_name += '_maxRR'

    if use_RR:
        feat_lab_patients_name += '_RR'
    
    if norm_RR:
        feat_lab_patients_name += '_norm_RR'

    for descp in compute_morph:
        feat_lab_patients_name += '_' + descp

    if reduced_DS:
        feat_lab_patients_name += '_reduced'
        
    if leads_flag[0] == 1:
        feat_lab_patients_name += '_MLII'

    if leads_flag[1] == 1:
        feat_lab_patients_name += '_V1'
    
    # if add_new and DS == 'DS1':
        # feat_lab_patients_name += '_new'

    if fake_train and DS == 'DS1':
        feat_lab_patients_name += '_fake'

    if fake_test and DS == 'DS2':
        feat_lab_patients_name += '_fake'

    if fake_train and DS == 'DS2':
        times = 1

    if fake_test and DS == 'DS1':
        times = 1

    feat_lab_patients_name += f'_{times}.p'

    return feat_lab_patients_name
...
def load_mit_db(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag, add_new=True, is_intra=True, times=None, fake_train=False, fake_test=False):
    feat_lab_patients_name = create_feat_lab_patients_name(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag, add_new, is_intra, times, fake_train, fake_test)
    print("feat_lab_patients_name: ", feat_lab_patients_name)

    if os.path.isfile(feat_lab_patients_name):
        print("Features_label file already exists! Loading pickle: " + feat_lab_patients_name + "...")
        f = open(feat_lab_patients_name, 'rb')
        # disable garbage collector
        gc.disable()# this improve the required loading time!

        # features, labels, patient_num_beats = pickle.load(f)
        features, labels, patient_num_beats = joblib.load(f)
        ...
        print(f"No existed feature_labels file:{feat_lab_patients_name}. Now it is being created. Loading MIT BIH arr (" + DS + ") ...")
        ...
        mit_pickle_name = db_path + 'preprocess/' + 'python_mit'
        if reduced_DS:
        ...
        mit_pickle_name = mit_pickle_name + '_' + DS
        if DS == 'DS1' and add_new:
            mit_pickle_name += '_new'
        mit_pickle_name += '.p'
        print("mit_pickle_name: ", mit_pickle_name)

        # If the data with that configuration has been already computed Load pickle
        if os.path.isfile(mit_pickle_name):
            print(f'{mit_pickle_name} exists!')
        ...
        if 'raw' in compute_morph:
            ...
            for p in range(len(my_db.beat)):
                ...
                for beat in my_db.beat[p]:
                    ...
                    patients_idx.append(int(my_db.filename[p].split('.')[0]))
                    f_raw = np.vstack((f_raw, f_raw_lead))
        ...
        labels = np.array(sum(my_db.class_ID, [])).flatten() # 标签
        print("labels: ", labels.shape)

        # Set labels array!
        f = open(feat_lab_patients_name, 'wb')
        # pickle.dump([features, labels, patient_num_beats], f, 2)
        joblib.dump([features, labels, patients_idx, patient_num_beats], f)
        f.close()
        # Save patients idx file
        # np.savetxt(f'../dataset/archive/intra_features/{DS}_patients_idx.csv', np.array(patients_idx), delimiter=',')
...
def load_signal(DS: List[int], winL, winR, do_preprocess):
    ''' 
    ...
    #print('Original_R_poses: ', Original_R_poses)
    ...
    pathDB = "dataset/"
    DB_name = '/archive/mitbih_database'
    fs = 360
    jump_lines = 1
    ...
    # lst: list = os.listdir(pathDB + DB_name + "/csv")
    lst: List[str] = os.listdir(pathDB + DB_name)
    lst.sort()
    ...
    for r in range(0, len(fRecords)):
        ...
        filename = pathDB + DB_name + '/' + fRecords[r] # ../100.csv
        ...
        filename = pathDB + DB_name + "/" + fAnnotations[r] # e.g. ../101.txt
        print('Reading annotations, file:', filename)
        f = open(filename, 'r')
        # f = open(filename, 'rb')
        next(f) # skip first line!

        annotations = []
        for line in f: # line is like:  0:00.214       77     N    0    0    0
        ...
        for a in annotations:
            print(f"Coping annotation: {a}...")
            # a is like: '0:00.214       77     N    0    0    0'
            aS: List[str] = a.split()  # [0:00.214, 77, N, 0, 0, 0]
            
            pos = int(aS[1]) # 77
            originalPos = int(aS[1]) # 77
            classAnttd = aS[2] # N
            ...
            if classAnttd in MITBIH_classes:
                if(pos > winL and pos < (len(MLII) - winR)):
                    #print(f'classAnttd: {classAnttd}, pos: {pos}\n')

                    beat[r].append((MLII[pos - winL : pos + winR], V1[pos - winL : pos + winR]))
                    for i in range(0, len(AAMI_classes)):
                        # AAMI_classes = [['N', 'L', 'R'], ['A', 'a', 'J', 'S', 'e', 'j'], ['V', 'E'], ['F']]
                        if classAnttd in AAMI_classes[i]:
                            # e.g. AAMI_classes[0] = ['N', 'L', 'R']
                            print('initial class label: ', classAnttd)
                            class_AAMI = i
                            break #exit loop
                    #convert class
                    print('converted class label: ', class_AAMI)
...
if __name__ == "__main__":
    db_path = '../dataset/archive/'
    load_mit_db('DS1', winL=128, winR=129, do_preprocess=True, maxRR=True, use_RR=True, norm_RR=False, compute_morph = ['raw'], db_path=db_path, reduced_DS=False, leads_flag=[1, 0])
    load_mit_db('DS2', winL=128, winR=129, do_preprocess=True, maxRR=True, use_RR=True, norm_RR=False, compute_morph = ['raw'], db_path=db_path, reduced_DS=False, leads_flag=[1, 0])